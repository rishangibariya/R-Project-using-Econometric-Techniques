...
model <- lm(Forex_Reserves ~ NEER_Import + FDI, data = data)

# Display summary of the model
summary(model)

# Plot the regression line for NEER Import (one predictor at a time for visualization)
plot(data$NEER_Import, data$Forex_Reserves, main = "Regression of Forex Reserves on NEER Import",
     xlab = "NEER Import", ylab = "Forex Reserves")
abline(lm(Forex_Reserves ~ NEER_Import, data = data), col = "blue")
